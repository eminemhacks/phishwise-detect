...
export function analyzeUrl(rawInput: string): UrlAnalysis {
  ...
  // Excessive subdomains (3+ labels in front of the registrable domain).
  if (subdomainLabels.length >= 3) {
    add(fire('url-excessive-subdomains', `${subdomainLabels.length} subdomain labels`));
  }

  // Random-looking / high-entropy host.
  if (!isIp && !isLegit && sld.length >= 12 && (digitRatio(sld) >= 0.33 || vowelRatio(sld) < 0.26)) {
    add(fire('url-high-entropy-host', `\u201c${sld}\u201d looks machine-generated`));
  }
